...
tc1[0].addEventListener('click', function (e){
    ...
    if(tc1.length>=3)
    {
        ...
        tc1[1].src = 'sources/5/1b.png';
        displayword(e, '关上柜门');
        var tmp = Number(sessionStorage.getItem('5_1'));
        if(tmp == 0)
        {
            sessionStorage.setItem('5_1', '1');
...
tc2[0].addEventListener('click', function (e){
    ...
    if(tc2.length>=3)
    {
        ...
        tc2[1].src = 'sources/5/2b.png';
        displayword(e, '关闭通风');

        var tmp = Number(sessionStorage.getItem('5_2'));
        if(tmp == 0)
        {
            sessionStorage.setItem('5_2', '1');
            tmp = Number(sessionStorage.getItem('count'));
            tmp = tmp + 1;
            sessionStorage.setItem('count', String(tmp));
            showScore();
...
tc3[0].addEventListener('click', function (e){
    ...
    if(tc3.length>=3)
    {
        ...
        tc3[1].src = 'sources/5/3b.png';
        displayword(e, '取出培养皿');
        var tmp = Number(sessionStorage.getItem('5_3'));
        if(tmp == 0)
        {
            sessionStorage.setItem('5_3', '1');
            tmp = Number(sessionStorage.getItem('count'));
            tmp = tmp + 1;
            sessionStorage.setItem('count', String(tmp));
            showScore();
